...
.. api-header::
   ...
   .. api-member::
      ...
      .. api-member::
         ...
         The message can either be returned as a DOM File (default) or as a `binary string <https://developer.mozilla.org/en-US/docs/Web/API/DOMString/Binary>`__. It is recommended to use the ``File`` format, because the DOM File object can be used as-is with the downloads API and has useful methods to access the content, like `File.text() <|link-DOMFile-text|>`__ and `File.arrayBuffer() <|link-DOMFile-arrayBuffer|>`__. Working with binary strings is error prone and needs special handling: 
